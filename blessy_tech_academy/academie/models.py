...
class LogAudit(models.Model):
    ...
    utilisateur = models.ForeignKey(
        "auth.User", on_delete=models.SET_NULL, null=True, related_name="logs_audit"
    )
...
class Enseignant(models.Model):
    ...
    profil = models.OneToOneField(
        ProfilUtilisateur, on_delete=models.CASCADE, related_name="enseignant"
    )
    formations_attribuees = models.ManyToManyField(
        Formation, blank=True, related_name="enseignants"
    )
    ...
    def nb_etudiants_formes(self):
        return (
            AccesFormationDebloque.objects.filter(formation__in=self.formations_attribuees.all())
            .values("utilisateur")
            .distinct()
            .count()
        )
